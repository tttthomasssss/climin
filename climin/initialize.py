...
def randn(views, random_state=None, scale=0.01):
	rng = np.random if random_state is None else random_state

	params = np.array([])

	for V in views:
		W = scale * rng.randn(V.shape[0], V.shape[1]) if V.ndim > 1 else scale * rng.randn(V.shape[0])
		params = np.concatenate((params, W.reshape(-1,)))

	return params


def randomize_normal_relu(params, random_state=None):
	return randomize_normal(params, 0, 0.01, random_state)